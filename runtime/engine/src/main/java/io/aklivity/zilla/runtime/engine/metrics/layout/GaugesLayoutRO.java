...
public final class GaugesLayoutRO extends MetricsLayoutRO
{
    private static final int RECORD_SIZE = 3 * FIELD_SIZE;

    private final GaugesLayout layout;

    private GaugesLayoutRO(
        GaugesLayout layout)
    {
    ...
    public static final class Builder extends LayoutRO.Builder<GaugesLayoutRO>
    {
        private Path path;
        ...
        @Override
        public GaugesLayoutRO build()
        {
            GaugesLayout layout = new GaugesLayout.Builder().path(path).mode(READ_ONLY).build();
            return new GaugesLayoutRO(layout);
        }
    }
}
